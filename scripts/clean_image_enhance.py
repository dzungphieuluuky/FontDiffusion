...
class CharacterImageCleaner:
    ...
    def remove_colored_background(self, img: np.ndarray) -> np.ndarray:
        ...
        if saturation.std() > 30:
            ...
            enhanced = gray.copy()
            enhanced[text_mask] = np.clip(enhanced[text_mask] * 1.2, 0, 255).astype(
                np.uint8
            )
    ...
    def binarize_adaptive(
        self, gray: np.ndarray, block_size: int = 25, c: int = 10
    ) -> np.ndarray:
    ...
    def binarize_sauvola(
        self, gray: np.ndarray, window_size: int = 25, k: float = 0.2
    ) -> np.ndarray:
        """
        ...
        sqmean = cv2.boxFilter(
            gray.astype(np.float32) ** 2, -1, (window_size, window_size)
        )
    ...
    def remove_noise(
        self, binary: np.ndarray, min_component_size: int = 20
    ) -> np.ndarray:
        """
    ...
    def center_character(self, binary: np.ndarray) -> Tuple[np.ndarray, bool]:
        ...
        resized_char = cv2.resize(
            character_crop, (new_w, new_h), interpolation=interpolation
        )
    ...
    def clean_image(
        self, image_path: Path, debug: bool = False
...
def visualize_cleaning_steps(
    image_path: Path, cleaner: CharacterImageCleaner, output_path: Optional[Path] = None
):
    """
...
Examples:
  ...
  python clean_style_images.py input_folder --size 512 --padding 50
    ...
    parser.add_argument("input_folder", type=str, help="Folder containing raw images")
    parser.add_argument(
        "--output",
        type=str,
