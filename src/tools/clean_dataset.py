...
class DatasetCleaner:
    ...
    def __init__(self, dataset_dir: str | Path, dry_run: bool = True):
        ...
        self.checkpoint_backup_path: Path = (
            self.dataset_dir / "results_checkpoint.json.backup"
        )
    ...
    def load_checkpoint(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"✓ Loaded checkpoint with {len(self.checkpoint_records)} records"
            )
            return True
    ...
    def identify_coverage(
        ...
        logger.info(
            f"Found {len(unique_styles)} unique styles: {sorted(unique_styles)}"
        )
        ...
        for char in unique_characters:
            covered_styles = sum(
                1 for s in unique_styles if coverage[char].get(s, False)
            )
        ...
        logger.info(f"\n📊 Coverage Summary:")
        logger.info(
            f"  Fully covered (all {len(unique_styles)} styles):  {len(fully_covered)}"
        )
    ...
    def get_files_to_remove(
        self, partially_covered_chars: set[str]
    ...
    def preview_deletion(
        ...
        logger.info(
            f"  Records to remove: {len([r for r in self.checkpoint_records if r.get('character') in partially_covered_chars])}"
        )
        logger.info(
            f"  Remaining records: {len(self.checkpoint_records) - len([r for r in self.checkpoint_records if r.get('character') in partially_covered_chars])}"
        )
    ...
    def perform_deletion(self, partially_covered_chars: set[str]) -> bool:
        ...
        self.stats["removed_records"] = original_record_count - len(
            self.checkpoint_records
        )
        ...
        if not self.dry_run:
            ...
            logger.info(
                f"[DRY RUN] Would update checkpoint with {len(self.checkpoint_records)} records"
            )
            return True
    ...
    def print_summary(self) -> None:
        ...
        logger.info(
            f"  Initial characters:           {self.stats['initial_characters']}"
        )
        logger.info(f"  Initial styles:               {self.stats['initial_styles']}")
        logger.info(f"")
        logger.info(
            f"  Fully covered characters:     {self.stats['fully_covered_chars']}"
        )
        logger.info(
            f"  Partially covered characters: {self.stats['partially_covered_chars']}"
        )
        logger.info(f"")
        logger.info(f"  Records removed:              {self.stats['removed_records']}")
        logger.info(
            f"  Content images removed:       {self.stats['removed_content_images']}"
        )
        logger.info(
            f"  Target images removed:        {self.stats['removed_target_images']}"
        )
        logger.info(f"")
