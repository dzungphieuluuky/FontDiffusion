"""
FontDiffuser Multi‑GPU Batch Sampler – clean, well‑structured implementation.

The functionality is unchanged; only formatting, import ordering, and
minor refactorings have been applied for readability.
"""
...
def generate_content_images_with_accelerator(
    ...
    Returns a dictionary mapping each character to its image path.
    """
    output_path = Path(output_dir)
    content_dir = output_path / "ContentImage"
    ...
    local_char_paths: Dict[str, str] = {}
    with accelerator.split_between_processes(characters) as local_chars:
        for char in local_chars:
            found_font = next(
                (fn for fn in font_names if font_manager.is_char_in_font(fn, char)),
                None,
            )
            if not found_font:
                continue
            ...
            try:
                ...
                logger.warning(
                    f"GPU {accelerator.process_index}: Error generating '{char}': {exc}"
                )

    accelerator.wait_for_everyone()
    all_char_paths_list = gather_object([local_char_paths])

    merged_char_paths: Dict[str, str] = {}
...
def sampling_batch_with_accelerator(
    ...
    font_manager: FontManager,
    font_name: str,
) -> Tuple[Union[List[Image.Image], None], Union[List[str], None], Union[float, None]]:
    """
    Batch sampling for multiple characters.

    Returns (generated_images, valid_characters, batch_time).
    """
    ...
    try:
        style_image = (
            ...
            else style_image_path.convert("RGB")
        )
        ...
        content_images = [
            content_transform(ttf2im(font=font, char=char))
            for char in available_chars
            if not logger.warning as e:
        ...
        logger.error(f"Batch sampling failed: {exc}")
        return None, None, None
...
def batch_generate_images_with_accelerator(
    ...
    results: Dict[str, Any] = {
        ...
        "characters": sorted(all_chars_in_checkpoint),
        "styles": sorted(all_styles_in_checkpoint),
    ...
    primary_font = font_manager.get_font_names()[0]
    if accelerator.is_main_process:
        logger.info(
            ...
            try:
                ...
                for char, img in zip(valid_chars, images):
                    try:
                        ...
                        logger.warning(
                            f"GPU {accelerator.process_index}: Error saving '{char}': {exc}"
                        )
                ...
                logger.error(
                    f"GPU {accelerator.process_index}: Error processing {style_name}: {exc}"
                )
                local_failed_count += len(chars_to_generate) if chars_to_generate else len(characters)
    ...
    if accelerator.is_main_process:
        ...
        results["characters"] = sorted(all_chars_in_checkpoint)
        results["styles"] = sorted(all_styles_in_checkpoint)
...
def evaluate_results_with_accelerator(
    ...
    evaluator: QualityEvaluator,
    output_dir: str,
    ground_truth_dir: Union[str, None],
    compute_fid: bool = False,
    accelerator: Accelerator | None = None,
) -> Dict[str, Any]:
    """
    Evaluate generated images on the main process.
    """
    ...
    for gen in results["generations"]:
        ...
        try:
            ...
            logger.warning(f"Error evaluating {char}/{style}: {exc}")
    ...
    if lpips_scores:
        ...
        logger.info(
            f"LPIPS: mean={results['metrics']['lpips']['mean']:.4f}"
        )
...
def main() -> None:
    ...
    try:
        ...
        logger.error(
            f"GPU {accelerator.process_index}: Fatal error: {exc}", exc_info=True
        )
        accelerator.wait_for_everyone()
        sys.exit(1)
    finally:
        ...
        try:
            ...
            logger.warning(
                f"GPU {accelerator.process_index}: Error during cleanup: {exc}"
            )
