...
class MinimalFontGenerator(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Resize to 96x96
        out = torch.nn.functional.interpolate(out, size=(96, 96), mode='bilinear', align_corners=False)
        return out
...
class TestOverfitSingleBatch:
    ...
    def test_encoder_overfits_batch(self, device, simple_encoder, optimizer_factory, loss_fn):
...
class TestLossDecreaseTrajectory:
    ...
    def test_learning_rate_impact(self, device, simple_encoder, optimizer_factory, loss_fn):
...
class TestMultipleBackwardPasses:
    ...
    def test_gradient_accumulation(self, device, simple_encoder):
        ...
        grad_after_first = [p.grad.clone() for p in encoder.parameters() if p.grad is not None]
        
        output2 = encoder(x)
        loss2 = loss_fn(output2, target)
        loss2.backward()
        
        grad_after_second = [p.grad.clone() for p in encoder.parameters() if p.grad is not None]
    ...
    def test_gradient_reset(self, device, simple_encoder):
        ...
        for param in encoder.parameters():
            if param.grad is not None:
                assert (param.grad == 0).all(), "Gradients should be zero after zero_grad()"
...
class TestOptimizerStepping:
    ...
    def test_gradient_accumulation_with_steps(self, device, simple_encoder, 
