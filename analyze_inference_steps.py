...
class InferenceStepsAnalyzer:
    ...
    def load_results(self) -> None:
        ...
        comparison_dirs = [d for d in self.ablation_dir.iterdir() 
    ...
    def _create_steps_matrix(self, metric_name: str, steps_list: List[int]) -> np.ndarray:
    ...
    def plot_metric_heatmaps(self, output_dir: Path) -> None:
        ...
        metric_info = {
            ...
            'histogram_distance': {'title': 'Histogram Distance (Lower is Better)', 'cmap': 'RdYlGn_r'},
        ...
        for metric_name, info in metric_info.items():
            ...
            ax.set_title(f'{info["title"]}\nInference Steps Ablation Heatmap', fontsize=14, fontweight='bold', pad=20)
    ...
    def plot_steps_trends(self, output_dir: Path) -> None:
        ...
        for idx, (metric_name, display_name) in enumerate(metric_info.items()):
            ...
            ax.errorbar(steps_list, means, yerr=stds, marker='o', linewidth=2, markersize=8, capsize=5, label='Mean ± Std')
            ax.fill_between(steps_list, np.array(means) - np.array(stds), np.array(means) + np.array(stds), alpha=0.2)
    ...
    def plot_quality_vs_efficiency(self, output_dir: Path) -> None:
        ...
        metric_info = {
            ...
            'histogram_distance': {'display': 'Histogram Distance (Lower Better)', 'direction': 'lower'},
