...
class FontDiffuserOptimized:
    def __init__(self, args):
        ...
        print(
            f"🚀 Initializing FontDiffuser on {self.device} with FP16 Optimization..."
        )

        # 1. Load Components
        unet = build_unet(args=args)
        unet.load_state_dict(
            torch.load(f"{args.ckpt_dir}/unet.pth", map_location="cpu")
        )

        style_encoder = build_style_encoder(args=args)
        style_encoder.load_state_dict(
            torch.load(f"{args.ckpt_dir}/style_encoder.pth", map_location="cpu")
        )
    ...
    def run_single_inference(self):
        ...
        if not args.content_image_path and not args.character_input:
            raise ValueError(
                "Content image path or character input is required for single inference."
            )
    ...
    def run_batch_inference(self):
        ...
        if self.cached_style_tensor is None:
            raise RuntimeError(
                "Style tensor must be cached before running batch inference."
            )
        ...
        for i, image_file in enumerate(sorted(image_files)):
            ...
            try:
                ...
                with torch.no_grad():
                    images = self.pipe.generate(
                        content_images=content_tensor,
                        style_images=self.cached_style_tensor.repeat(
                            batch_size, 1, 1, 1
                        ),  # Repeat cached tensor
                ...
                print(
                    f"[{i + 1}/{len(image_files)}] Processed '{image_file}' (Time: {end - start:.2f}s) -> Saved to {output_path}"
                )
...
def arg_parse():
    ...
    args.save_image = (
        bool(args.content_image_path or args.character_input) and not args.content_dir
    )
...
if __name__ == "__main__":
    ...
    # 2. Check if running in single file/char mode
        ...
        print("  --style_image_path <path/to/style.png>")
        print(
            "  --content_image_path <path/to/content.png> OR --character_input --content_character 'A'"
        )
