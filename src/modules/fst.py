...
class FontStyleTransformationModule(nn.Module):
    ...
    def __init__(
        ...
        self.q_projs = nn.ModuleList([nn.Linear(query_dim, query_dim) for _ in range(num_scale_features)])
        self.k_projs = nn.ModuleList([nn.Linear(ch, query_dim) for ch in feature_channels])
        self.v_projs = nn.ModuleList([nn.Linear(ch, query_dim) for ch in feature_channels])
        ...
        total_concat_dim = query_dim * num_scale_features  # After concatenating all L_{x→y}^i
    ...
    def forward(
        ...
        output = torch.cat([L_transformed, last_feature_proj], dim=1)  # (B, N_L + H*W, 1024)
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.tensor, context: torch.tensor = None, value: torch.tensor = None):
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x: torch.tensor, context: torch.tensor, value: torch.tensor = None) -> torch.tensor:
        if value is None:
            value = context
        ...
        k = self.to_k(context).reshape(B, M, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        v = self.to_v(value).reshape(B, M, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
