...
class ValidationSplitCreator:
    ...
    def analyze_data(self) -> Tuple[List[str], List[str], Dict[str, List[str]]]:
        ...
        print(f"  Characters: {len(chars_list)} - {chars_list[:10]}{'...' if len(chars_list) > 10 else ''}")
    ...
    def create_validation_scenarios(self, 
                                   styles: List[str], 
                                   characters: List[str],
                                   char_to_styles: Dict[str, List[str]]) -> Dict[str, Dict]:
        """
        ...
        val_styles = set(shuffled_styles[num_train_styles:num_train_styles + num_val_styles])
        test_styles = set(shuffled_styles[num_train_styles + num_val_styles:])
        
        train_chars = set(shuffled_chars[:num_train_chars])
        val_chars = set(shuffled_chars[num_train_chars:num_train_chars + num_val_chars])
        ...
        print("\n📊 Split Statistics:")
        print(f"  Styles: {num_train_styles} train + {num_val_styles} val + {num_test_styles} test")
        print(f"  Chars:  {num_train_chars} train + {num_val_chars} val + {num_test_chars} test")
        ...
        for scenario_name, scenario_data in scenarios.items():
            ...
            print(f"    Chars: {scenario_data['characters'][:5]}{'...' if len(scenario_data['characters']) > 5 else ''}")
        
        return scenarios
    
    def copy_images_for_split(self, 
                             split_name: str,
                             split_dir: Path,
                             scenarios: Dict[str, Dict]) -> int:
    ...
    def create_splits(self) -> None:
        ...
        if self.config.create_scenarios:
            ...
            val_combined_scenarios = {
                'val': {
                    'styles': scenarios['val_unseen_both']['styles'] + scenarios['val_unseen_style_seen_char']['styles'],
                    'characters': scenarios['val_unseen_both']['characters'] + scenarios['val_unseen_style_seen_char']['characters']
                }
            }
