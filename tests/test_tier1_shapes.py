...
class TestMultiScaleStyleEncoderShapes:
    ...
    def test_msse_output_shapes(self, batch_size: int, num_scales: int, device, assert_shape, assert_no_nan):
        ...
        for i, feat in enumerate(features):
            assert feat.shape[0] == batch_size, \
                f"Scale {i}: batch size mismatch"
            assert feat.shape[1] > 0, \
                f"Scale {i}: channel count must be positive"
            assert feat.ndim == 4, \
                f"Scale {i}: feature must be 4D (B, C, H, W)"
            assert_no_nan(feat, f"MSSE scale {i}")
    
    @pytest.mark.parametrize("in_channels", [1, 3])
    def test_msse_variable_input_channels(self, in_channels: int, device, assert_no_nan):
...
class TestFSTModuleShapes:
    ...
    @pytest.mark.parametrize("batch_size,num_queries", [
        (1, 128),
        (2, 256),
        (4, 256),
    ])
    def test_fst_output_shape(self, batch_size: int, num_queries: int, device, assert_no_nan):
        ...
        for ch in feature_channels:
            ...
            src_feat = torch.randn(batch_size, ch, spatial_size, spatial_size, device=device)
            tgt_feat = torch.randn(batch_size, ch, spatial_size, spatial_size, device=device)
    ...
    def test_fst_gradient_flow(self, device):
        ...
        for ch in feature_channels:
            spatial_size = spatial_size // 2
            src_feat = torch.randn(batch_size=2, channels=ch, height=spatial_size, 
                                  width=spatial_size, device=device, requires_grad=True)
            tgt_feat = torch.randn(batch_size=2, channels=ch, height=spatial_size, 
                                  width=spatial_size, device=device, requires_grad=True)
            source_features.append(src_feat)
            target_features.append(tgt_feat)
...
class TestResidualBlockShapes:
    ...
    @pytest.mark.parametrize("in_channels,out_channels,downsample", [
        (64, 128, True),
        (128, 128, False),
        (256, 512, True),
    ])
    def test_residual_block_shapes(self, in_channels: int, out_channels: int, 
                                   downsample: bool, device, assert_no_nan):
