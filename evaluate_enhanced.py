...
class Visualizer:
    ...
    def plot_metric_distributions(self, results: Dict, metric_name: str):
        ...
        if metric_name not in results['per_style_metrics'][styles[0]].get('metrics', {}):
            return
        ...
        stds = []
        for style in styles:
            style_data = results['per_style_metrics'][style]['metrics'].get(metric_name, {})
            if style_data:
        ...
        if all_values:
            ...
            ax2.set_title(f'{metric_name.replace("_", " ").title()} Distribution by Style')
    ...
    def plot_tsne_embeddings(self, results: Dict):
        ...
        for i, (emb, style, source) in enumerate(zip(embeddings, all_labels, all_sources)):
            marker = 'o' if source == 'Folder1' else '^'
            ax.scatter(emb[0], emb[1], c=[style_to_color[style]], marker=marker, 
    ...
    def create_summary_report(self, results: Dict):
        ...
        if 'statistical_tests' in results:
            ...
            for metric_name, tests in results['statistical_tests'].items():
                html_content += f"<h3>{metric_name.replace('_', ' ').title()}</h3><table>"
                html_content += "<tr><th>Test</th><th>Statistic</th><th>P-Value</th><th>Significant</th></tr>"
                
                for test_name, test_data in tests.items():
                    if isinstance(test_data, dict):
                        sig_class = 'significant' if test_data.get('significant', False) else ''
                        html_content += f"""
                            <tr>
...
def evaluate_folders_enhanced(
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        ...
        for future in HFTqdm(as_completed(futures), total=len(work_items), 
                            desc="🔄 Computing metrics", colour="cyan"):
    ...
    for style in common_styles:
        ...
        for metric_name, values in style_metrics[style].items():
            if values:
                style_data['metrics'][metric_name] = analyzer.compute_summary_stats(values)
    ...
    try:
        ...
        if not args.skip_visualizations:
            ...
            if results['metrics']:
                ...
                key_metrics = [m for m in ['lpips', 'ssim', 'psnr', 'mse'] if m in available_metrics]
                if key_metrics:
