...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--style_dir", type=str, required=True,
    ...
    parser.add_argument(
        "--resize_height", type=int, default=256,
    ...
    parser.add_argument(
        "--match_by", type=str, default="name",
        choices=["name", "index"],
        help="How to match images: 'name' (by filename) or 'index' (by sorted order)"
    )
    parser.add_argument(
        "--workers", type=int, default=os.cpu_count() - 1,
...
def match_images_by_index(
    ...
    return list(zip(
        content_files[:min_len],
        style_files[:min_len],
        target_files[:min_len]
    ))


def create_comparison_worker(args_tuple: tuple) -> tuple[bool, Path, Optional[str]]:
    """
    Worker function for creating a comparison image.
    
    Args:
        args_tuple: (content_path, style_path, target_path, output_path, resize_height, spacing)
    ...
    try:
        ...
        total_width = content_img.width + style_img.width + target_img.width + 2 * spacing
...
def main() -> None:
    ...
    directories = [(content_dir, "content"), (style_dir, "style"), (target_dir, "target")]
    ...
    for content_path, style_path, target_path in matched:
        ...
        work_items.append((
            content_path, style_path, target_path,
            output_path, args.resize_height, args.spacing
        ))
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        # Submit all tasks
        futures = {
            executor.submit(create_comparison_worker, item): item 
        ...
        for future in as_completed(futures):
            ...
            if success:
                ...
                logger.error(f"Failed to create {output_path.name}: {error_msg}")
                error_count += 1
