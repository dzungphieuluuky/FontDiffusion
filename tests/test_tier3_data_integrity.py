...
class TestDatasetShuffling:
    ...
    def test_dataloader_shuffle_true_reorders(self):
        ...
        num_different_positions = sum(1 for a, b in zip(indices_no_shuffle, indices_shuffle) if a != b)
...
class TestLabelLeakage:
    ...
    def test_content_not_in_style_input(self):
        ...
        for idx in range(len(dataset)):
            ...
            assert content is not style, \
                "Content and style should be different objects"
    ...
    def test_target_not_in_inputs(self):
        ...
        for batch in loader:
            ...
            assert target.shape[1] == 1, \
                "Target should not be concatenated with inputs"
...
class TestCollateFunction:
    ...
    def test_collate_preserves_dtype(self):
        ...
        batch = [
            {
                'content_image': torch.rand(1, 96, 96, dtype=torch.float32),
                'index': 0
            },
            {
                'content_image': torch.rand(1, 96, 96, dtype=torch.float32),
...
class TestDataLoaderDeterminism:
    ...
    def test_dataloader_same_seed_same_order(self):
        ...
        assert data1 == data2, \
            "DataLoader with same seed should produce same order"
    ...
    def test_full_pipeline_no_leakage(self):
        ...
        for batch_idx, batch in enumerate(loader):
            ...
            for i in range(combined_flat.shape[0]):
                corr = torch.corrcoef(
                    torch.stack([combined_flat[i], target_flat[i]])
                )[0, 1]
