...
class FontDiffuserFSTTrainer(FontDiffuserTrainer):
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        self.freeze_modules = self._parse_freeze_modules(
            getattr(args, "freeze_modules", "")
        )
        ...
        for k, v in {
            ...
            "frequency_normalize_bands": True,
    ...
    def _parse_freeze_modules(self, modules_str: str) -> list[str]:
        if not modules_str or not modules_str.strip():
            return []
        valid = {
            "unet",
            "style_encoder",
    ...
    def _setup_models(self):
        ...
        if self.use_fst:
            ...
            if self.use_skeleton_content:
                content_encoder = build_dual_channel_content_encoder(self.args)
            frequency_decomp = (
            ...
            if self.use_skeleton_content:
                logger.info(
                    f"Skeleton Config: method={self.skeleton_method}, dist={self.skeleton_distance_method}"
                )
            if self.use_frequency_decomp:
                logger.info(
                    f"Frequency Config: low={self.frequency_low_cutoff}, mid={self.frequency_mid_cutoff}"
                )
            ...
            if self.identity_loss_module:
                logger.info(f"✓ IdentityMappingLoss created")
        else:
    ...
    def _setup_logging(self):
        ...
        if self.use_fst:
            ...
            cfg["model_info"] = {
                f"{n}_params": sum(
                    p.numel() for p in getattr(unwrapped, n).parameters()
                )
                for n in ["mss_encoder", "fst_module", "fst_projection"]
            }
    ...
    def _apply_module_freezing(self):
        ...
        if self.use_fst:
            module_map = {
                "unet": self.model.diffusion_unet,
                "style_encoder": self.model.style_encoder,
                "content_encoder": self.model.content_encoder,
                "mss_encoder": self.model.mss_encoder,
                "fst_module": self.model.fst_module,
                "fst_projection": self.model.fst_projection,
                "original_style_projection": self.model.original_style_projection,
            }
        else:
            module_map = {
                "unet": self.model.config.unet,
                "style_encoder": self.model.config.style_encoder,
                "content_encoder": self.model.config.content_encoder,
            }
    ...
    def _load_phase1_checkpoints(self, unet, style_encoder, content_encoder, ckpt_dir):
        ...
        for name, comp in [
            ...
            try:
                ...
                if path.exists():
                    comp.load_state_dict(load_model_checkpoint(path))
                    logger.info(f"✓ Loaded {name}")
                else:
                    logger.warning(f"[WARNING] {name} not found")
        ...
        if self.use_fst:
            ...
            for name in [
                ...
                try:
                    path = find_checkpoint(ckpt_dir, name)
                    if path.exists():
                        self._fst_module_states[name] = load_model_checkpoint(path)
                except:
                    pass
    ...
    def _wrap_components(self):
        ...
        if (
            self.use_fst
            and hasattr(self, "identity_loss_module")
            and self.identity_loss_module
        ):
        ...
        if hasattr(self, "_fst_module_states") and self.use_fst:
            ...
            for name, state in self._fst_module_states.items():
                if hasattr(unwrapped, name):
                    getattr(unwrapped, name).load_state_dict(state)
    ...
    def apply_classifier_free_guidance(
        self, content_images, style_images, drop_prob, samples=None
    ):
        ...
        for i, mask in enumerate(context_mask):
            if mask == 1:
                ...
                if samples and "style_source_image" in samples:
                    samples["style_source_image"][i] = 1.0
