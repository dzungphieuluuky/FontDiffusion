...
class TestFSTModuleIntegration:
    ...
    def test_fst_forward_backward_flow(self, device):
        ...
        for ch in feature_channels:
            ...
            src = torch.randn(batch_size, ch, spatial_size, spatial_size, device=device, requires_grad=True)
            tgt = torch.randn(batch_size, ch, spatial_size, spatial_size, device=device, requires_grad=True)
    ...
    @pytest.mark.parametrize("num_queries,query_dim", [
        (64, 64),
        (128, 128),
        (256, 128),
    ])
...
class TestMSSEIntegration:
    ...
    def test_msse_training_mode_variance(self, device):
        ...
        all_same = all(torch.allclose(features_list[0], f, atol=1e-6) for f in features_list[1:])
    ...
    def test_fst_projection_layers_gradients(self, device):
        ...
        for name, param in fst.mlp_channel_adjust.named_parameters():
            assert param.grad is not None, \
                f"MLP param {name} has no gradient"
...
class TestMSSEOverfitting:
    ...
    def test_msse_can_memorize(self, device):
        ...
        for _ in range(3):
            spatial_size = spatial_size // 2
            target_features.append(torch.randn(2, 32 * (2 ** _), spatial_size, spatial_size, device=device))
