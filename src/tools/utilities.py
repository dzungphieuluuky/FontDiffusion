...
def log_model_info(model: torch.nn.Module, name: str = "Model") -> None:
    ...
    logger.info(
        f"|  {'Trainable Parameters':<30} {_format_param_count(trainable):>16}  |"
    )
...
def update_paths(inp: Path | str, out: Optional[Path | str] = None) -> None:
    inp = _ensure_path(inp)
    out = _ensure_path(out) if out else inp
...
def print_font_glyph_counts(dir_: Path | str) -> None:
    ...
    for fp in fonts:
        try:
            ...
            print(f"  {fp.name}: Failed to read ({e})")
...
def convert_checkpoint_folder(
    src_dir: Path | str, dst_dir: Optional[Path | str] = None
) -> Path:
    src_dir = _ensure_path(src_dir)
    dst_dir = _ensure_path(dst_dir) if dst_dir else src_dir
...
def _cli() -> None:
    ...
    parser.add_argument(
        "--output_dir", type=str, default=None, help="Output dir for safetensors"
    )
    parser.add_argument(
        "--rename_images_json", type=str, help="Rename images from JSON"
    )
    parser.add_argument(
        "--rename_content_images_dir", type=str, help="Rename content images"
    )
