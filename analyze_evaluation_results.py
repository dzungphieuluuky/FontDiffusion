...
METRIC_PROPERTIES = {
    ...
    MetricType.HISTOGRAM: {"direction": MetricDirection.LOWER_BETTER, "display": "Histogram Distance"},
...
class EvaluationAnalyzer:
    ...
    def analyze_global_metrics(self) -> Dict[str, MetricStats]:
        ...
        for metric_name, metric_data in metrics.items():
            ...
            if stats:
                ...
                display_name = METRIC_PROPERTIES.get(
                    MetricType(metric_name), {}
                ).get("display", metric_name.upper().replace("_", " "))
        ...
        for style_name, style_data in sorted(per_style.items()):
            ...
            for metric_type in MetricType:
                ...
                if metric_name in style_data:
                    metric_stats = self.validator.clean_metric_dict(style_data[metric_name])
                    
                    if metric_stats:
                        style_results[metric_name] = metric_stats
                        display_name = METRIC_PROPERTIES[metric_type]["display"]
                        print(f"  {display_name}: {metric_stats.mean:.4f} ± {metric_stats.std:.4f} (n={metric_stats.count})")
                        
                        # Track for ranking
                        metric_rankings[metric_name].append((style_name, metric_stats.mean))
    ...
    def _print_style_rankings(self, rankings: Dict[str, List[Tuple[str, float]]]) -> None:
        ...
        for metric_name, style_values in rankings.items():
            ...
            try:
                ...
                sorted_styles = sorted(style_values, key=lambda x: x[1], reverse=reverse)
                
                print(f"📊 {display_name} ({'higher is better' if reverse else 'lower is better'}):")
    ...
    def analyze_per_image_metrics(self) -> Dict[str, Tuple[MetricStats, OutlierInfo]]:
        ...
        for img_data in per_image:
            ...
            for metric_type in MetricType:
                ...
                if metric_name in img_data:
                    val = self.validator.validate_metric_value(img_data[metric_name], metric_name)
                    if val is not None:
        ...
        for metric_name, values in metrics_data.items():
            ...
            outliers = self.stat_analyzer.detect_outliers(values, method="iqr", threshold=1.5)
    ...
    def analyze_cross_style_consistency(self) -> Dict[str, MetricStats]:
            ...
            for metric_type in MetricType:
                ...
                if metric_name in style_data:
                    ...
                    if isinstance(metric_dict, dict) and "mean" in metric_dict:
                        val = self.validator.validate_metric_value(metric_dict["mean"], metric_name)
                        if val is not None:
        ...
        for metric_name, means in cross_style_means.items():
            ...
            if stats:
                ...
                consistency = "High" if stats.cv < 10 else "Medium" if stats.cv < 25 else "Low"
    ...
    def _plot_violin_plots(self, df: pd.DataFrame, output_dir: Path) -> None:
        ...
        plt.tight_layout()
        plt.savefig(output_dir / 'metric_violin_plots.png', dpi=150, bbox_inches='tight')
        plt.close()
...
"""
            ...
            for style_name, metrics in sorted(per_style_stats.items()):
                ...
                for metric_type in [MetricType.LPIPS, MetricType.SSIM, MetricType.HISTOGRAM, MetricType.FID]:
                    metric_name = metric_type.value
...
"""
        ...
        if per_image_stats:
            for metric_name, (stats, outliers) in per_image_stats.items():
                ...
                outlier_badge = ""
                if outliers.count > 0:
                    badge_class = "badge-warning" if outliers.percentage < 5 else "badge-danger"
...
"""
        ...
        for metric_name, stats in consistency_stats.items():
            ...
            consistency = "High" if stats.cv < 10 else "Medium" if stats.cv < 25 else "Low"
            badge_class = "badge-success" if stats.cv < 10 else "badge-warning" if stats.cv < 25 else "badge-danger"
...
"""
    ...
    def run_full_analysis(self, output_dir: Union[str, Path] = "analysis_results") -> None:
