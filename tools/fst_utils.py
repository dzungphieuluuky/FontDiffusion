...
class ModelCheckpointManager:
    ...
    def load_fst_checkpoint(
        ...
        if (ckpt_dir / "style_encoder.pth").exists():
            model.style_encoder.load_state_dict(torch.load(ckpt_dir / "style_encoder.pth"))
        if (ckpt_dir / "content_encoder.pth").exists():
            model.content_encoder.load_state_dict(torch.load(ckpt_dir / "content_encoder.pth"))
        ...
        if (ckpt_dir / "fst_projection.pth").exists():
            model.fst_projection.load_state_dict(torch.load(ckpt_dir / "fst_projection.pth"))
...
class ModelComparator:
    ...
    def compare_outputs(
        ...
        comparison = {
            ...
            'fst_transformation_features_shape': tuple(fst_outputs['transformation_features'].shape),
            'fst_additional_outputs': len(fst_outputs) - 2,  # Excluding noise_pred and offset
        }
                ...
                if is_fst:
                    outputs = model(
                        noisy_latents=torch.randn(
                            content_img.shape[0], 4, 24, 24
                        ).to(self.device),
                        timestep=torch.randint(0, 1000, (content_img.shape[0],)).to(self.device),
...
def visualize_fst_features(
    transformation_features: torch.Tensor,
