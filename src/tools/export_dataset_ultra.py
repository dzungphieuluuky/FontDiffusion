...
def _atomic_save_image(img: Image.Image, path: Path) -> None:
    ...
    try:
        with os.fdopen(fd, "wb") as f: img.save(f, format=img.format or "PNG")
        os.replace(tmp, path)
    except:
        os.unlink(tmp); raise
...
def _process_batch_export(batch: dict, output_dir: str) -> dict:
    ...
    results = {k: [] for k in ["content_saved", "target_saved", "content_filename", "target_filename", "content_hash", "target_hash"]}
    ...
    for i in range(size):
        ...
        results["content_filename"].append(c_fn); results["target_filename"].append(t_fn)
        ...
        if isinstance(c_img, Image.Image):
            try: _atomic_save_image(c_img, out_path / "ContentImage" / c_fn); c_saved = True
            except Exception as e: logger.debug(f"Err content {c_fn}: {e}")
        ...
        if isinstance(t_img, Image.Image):
            try: _atomic_save_image(t_img, out_path / "TargetImage" / style / t_fn); t_saved = True
            except Exception as e: logger.debug(f"Err target {t_fn}: {e}")
...
class ExportConfig:
    ...
    def __post_init__(self):
        ...
        if self.local_dataset_path: self.local_dataset_path = Path(self.local_dataset_path)
        if not self.repo_id and not self.local_dataset_path: raise ValueError("Require repo_id or local_dataset_path")
...
class DatasetExporter:
    ...
    def _load_dataset(self) -> Dataset:
        if self.cfg.local_dataset_path:
            ...
            ds = load_dataset(self.cfg.repo_id, name=self.cfg.config_name, split=self.cfg.split, token=self.cfg.token, num_proc=self.num_workers)
    ...
    def _export_with_map(self, dataset: Dataset) -> dict[str, Any]:
        ...
        processed = dataset.map(
            _process_batch_export, batched=True, batch_size=self.cfg.batch_size,
            num_proc=self.num_workers, desc="Exporting", fn_kwargs={"output_dir": str(self.cfg.output_dir)}
        )
        ...
        with ckpt_path.open("w", encoding="utf-8") as f:
            ...
            for i, s in enumerate(processed):
                ...
                chars.add(c); styles.add(st); fonts.add(fn)
                
                if s.get("target_saved"):
                    rec = {
                        "character": c, "style": st, "font": fn,
                        "content_image_path": f"ContentImage/{s['content_filename']}",
                        "target_image_path": f"TargetImage/{st}/{s['target_filename']}",
                        "content_hash": s["content_hash"], "target_hash": s["target_hash"]
                    }
                    if gen_count > 0: f.write(",\n")
                    f.write("    " + json.dumps(rec, ensure_ascii=False))
                    gen_count += 1
                
                if (i + 1) % 1000 == 0: logger.info(f"Progress: {i+1}/{len(processed)}")
            
            f.write(f'\n  ],\n  "characters": {json.dumps(sorted(chars))},\n')
            f.write(f'  "styles": {json.dumps(sorted(styles))},\n')
            f.write(f'  "fonts": {json.dumps(sorted(fonts))},\n')
            f.write(f'  "total_chars": {len(chars)},\n  "total_styles": {len(styles)}\n}}\n')

        content_count = len(list((self.cfg.output_dir / "ContentImage").glob("*.png")))
        return {"generations_count": gen_count, "content_count": content_count, "total_chars": len(chars), "total_styles": len(styles), "characters": sorted(chars), "styles": sorted(styles), "fonts": sorted(fonts)}

    def export(self) -> dict[str, Any]:
        ds = self._load_dataset()
        # Pre-create dirs
        (self.cfg.output_dir / "ContentImage").mkdir(parents=True, exist_ok=True)
        for s in set(ds["style"]): (self.cfg.output_dir / "TargetImage" / s).mkdir(parents=True, exist_ok=True)
        return self._export_with_map(ds)

def export_dataset(output_dir: str | Path, repo_id: Optional[str] = None, local_dataset_path: Optional[str | Path] = None, **kwargs) -> dict[str, Any]:
    cfg = ExportConfig(output_dir=output_dir, repo_id=repo_id, local_dataset_path=local_dataset_path, **kwargs)
...
def main():
    ...
    try:
        meta = export_dataset(
            output_dir=args.output_dir, repo_id=args.repo_id, local_dataset_path=args.local_path,
            split=args.split, config_name=args.config_name, token=args.token,
            num_workers=args.workers, batch_size=args.batch_size
        )
        print(f"\n✅ Done! Content: {meta['content_count']}, Targets: {meta['generations_count']}")
    except Exception as e:
        logger.exception(f"Failed: {e}"); raise SystemExit(1)
