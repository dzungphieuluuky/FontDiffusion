...
class IdentityMappingLoss(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "identity_matrix", torch.eye(matrix_size, dtype=torch.float32)
        )
    ...
    def orthogonality_regularization(
        ...
        # Compute T^T @ T
        TtT = torch.bmm(
            transformation_matrix.transpose(1, 2), transformation_matrix
        )  # (B, N, N)
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            off_diagonal = T - torch.eye(N, device=T.device).unsqueeze(
                0
            ) * diagonal.unsqueeze(-1)

            metrics["diagonal_mean"] = diagonal.mean().item()
            metrics["diagonal_std"] = diagonal.std().item()
            metrics["off_diagonal_norm"] = (
                torch.norm(off_diagonal.reshape(B, -1), dim=1).mean().item()
            )
...
class PooledIdentityMappingLoss(nn.Module):
    ...
    def forward(
        ...
        if len(pair_features) == 0:
            return (torch.tensor(0.0, requires_grad=True), {"num_pairs": 0})
...
def example_usage():
    ...
    pairs = [
        (
            torch.randn(2, num_queries, feature_dim),
            torch.randn(2, num_queries, feature_dim),
        ),
        (
            torch.randn(2, num_queries, feature_dim),
