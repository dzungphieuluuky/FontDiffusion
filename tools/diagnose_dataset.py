...
class DatasetDiagnostics:
    ...
    def validate_structure(self) -> bool:
        ...
        if not self.target_base_dir.exists():
            ...
            logger.info(
                f"✓ TargetImage directory exists ({style_count} styles, {target_count} images)"
            )
    ...
    def load_checkpoint(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"✓ Loaded checkpoint with {len(self.checkpoint_records)} generation records"
            )
            ...
            if invalid_records > 0:
                logger.warning(
                    f"⚠ Found {invalid_records} records with missing required fields"
                )
    ...
    def verify_checkpoint_files(self) -> None:
        ...
        if missing_content > 0:
            logger.warning(
                f"⚠ Missing {missing_content} content images referenced in checkpoint"
            )
        if missing_target > 0:
            logger.warning(
                f"⚠ Missing {missing_target} target images referenced in checkpoint"
            )
    ...
    def find_extra_files(self) -> None:
        ...
        if self.content_dir.exists():
            ...
            if extra_content:
                logger.warning(
                    f"⚠ Found {len(extra_content)} extra content images not in checkpoint:"
                )
        ...
        if self.target_base_dir.exists():
            ...
            if extra_target:
                logger.warning(
                    f"⚠ Found {len(extra_target)} extra target images not in checkpoint:"
                )
    ...
    def diagnose_coverage(
        ...
        logger.info(
            f"Found {len(unique_styles)} unique styles and {len(unique_characters)} unique characters"
        )
    ...
    def print_coverage_summary(self, coverage: dict[str, dict[str, bool]]) -> None:
        ...
        for style in styles:
            ...
            covered_chars = sum(
                1 for char in coverage if coverage[char].get(style, False)
            )
    ...
    def print_issues(self) -> None:
        ...
        if self.issues["checkpoint_records_missing_fields"]:
            ...
            for issue in self.issues["checkpoint_records_missing_fields"][:5]:
                logger.error(
                    f"  • Record {issue['record_index']}: missing {issue['missing_fields']}"
                )
            if len(self.issues["checkpoint_records_missing_fields"]) > 5:
                logger.error(
                    f"  ... and {len(self.issues['checkpoint_records_missing_fields']) - 5} more"
                )
        ...
        if self.issues["missing_content_images"]:
            ...
            for issue in self.issues["missing_content_images"][:10]:
                logger.warning(
                    f"  • '{issue['character']}' / {issue['style']}: {issue['expected_path']}"
                )
            if len(self.issues["missing_content_images"]) > 10:
                logger.warning(
                    f"  ... and {len(self.issues['missing_content_images']) - 10} more"
                )
        ...
        if self.issues["missing_target_images"]:
            ...
            if len(self.issues["missing_target_images"]) > 10:
                logger.warning(
                    f"  ... and {len(self.issues['missing_target_images']) - 10} more"
                )
    ...
    def generate_report(self, output_file: str | Path | None = None) -> dict:
        ...
        report: dict = {
            ...
            "issues": {
                ...
                "checkpoint_records_missing_fields": self.issues[
                    "checkpoint_records_missing_fields"
                ],
            },
        }
    ...
    def run_full_diagnosis(self, output_file: str | Path | None = None) -> dict:
