...
class ConfigSweepRunner:
    ...
    def run_config(
        ...
        logger.info("=" * 80)
        logger.info(
            f"[{config_idx + 1}/{total_configs}] Running: {config.get_display_name()}"
        )
    ...
    def run_sweep(self, configs: List[InferenceConfig]) -> List[SweepResult]:
        ...
        for idx, config in enumerate(
            HFTqdm(configs, desc="🔄 Running sweep", colour="cyan")
        ):
...
class SweepComparator:
    ...
    def find_best_config(
        self, metric: str = "lpips_mean", lower_is_better: bool = True
    ...
    def generate_summary_report(self, output_dir: Path) -> None:
            ...
            if best:
                ...
                if best.metrics:
                    if metric in [f"{k}_mean" for k in best.metrics.keys()]:
                        val = best.metrics[metric.replace("_mean", "")].get(
                            "mean", "N/A"
                        )
        ...
        if self.results:
            inference_times = [
                r.inference_time for r in self.results if r.inference_time > 0
            ]
            if inference_times:
                report_lines.append(
                    f"Avg inference time: {np.mean(inference_times):.2f}s"
                )
                report_lines.append(
                    f"Min inference time: {np.min(inference_times):.2f}s"
                )
                report_lines.append(
                    f"Max inference time: {np.max(inference_times):.2f}s"
                )
...
Examples:
  ...
  python FontDiffusion/run_config_sweep.py \\
    ...
    parser.add_argument(
        "--output", type=str, default="sweep_results", help="Base output directory"
    )
    parser.add_argument(
        "--num_processes", type=int, default=4, help="Number of GPU processes"
    )
    parser.add_argument(
        "--dry_run",
        action="store_true",
    ...
    parser.add_argument(
        "--ckpt_dir", type=str, default="ckpt", help="Checkpoint directory"
    )
    parser.add_argument(
        "--characters", type=str, default="data/characters.txt", help="Characters file"
    )
    parser.add_argument(
        "--style_images",
        type=str,
    ...
    parser.add_argument(
        "--ttf_path", type=str, default="data/fonts/", help="TTF fonts directory"
    )
