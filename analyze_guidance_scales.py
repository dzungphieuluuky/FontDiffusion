...
class AblationAnalyzer:
    ...
    def plot_metric_heatmaps(self, output_dir: Path) -> None:
        ...
        for metric_name, info in metric_info.items():
            ...
            ax.set_title(f'{info["title"]}\nAblation Study Heatmap', fontsize=14, fontweight='bold', pad=20)
    ...
    def plot_scale_trends(self, output_dir: Path) -> None:
        ...
        scale_metrics: Dict[int, Dict[str, List[float]]] = defaultdict(lambda: defaultdict(list))
        ...
        for idx, (metric_name, display_name) in enumerate(metric_info.items()):
            ...
            ax.errorbar(scales, means, yerr=stds, marker='o', linewidth=2, markersize=8, capsize=5, label='Mean ± Std')
            ax.fill_between(scales, np.array(means) - np.array(stds), np.array(means) + np.array(stds), alpha=0.2)
