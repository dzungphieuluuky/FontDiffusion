...
def load_fontdiffuser_fst_pipeline(args):
    ...
    unet.load_state_dict(torch.load(Path(args.ckpt_dir) / "unet.pth"))
    style_encoder = build_style_encoder(args=args)
    style_encoder.load_state_dict(torch.load(Path(args.ckpt_dir) / "style_encoder.pth"))
    content_encoder = build_content_encoder(args=args)
    content_encoder.load_state_dict(torch.load(Path(args.ckpt_dir) / "content_encoder.pth"))
    # Create base model
    ...
    fst_dir = Path(fst_ckpt_dir)
    if (fst_dir / "mss_encoder.pth").exists():
        model.mss_encoder.load_state_dict(torch.load(fst_dir / "mss_encoder.pth"))
        print("Loaded MSSE encoder state_dict")
    if (fst_dir / "fst_module.pth").exists():
        model.fst_module.load_state_dict(torch.load(fst_dir / "fst_module.pth"))
        print("Loaded FST module state_dict")
    if (fst_dir / "fst_projection.pth").exists():
        model.fst_projection.load_state_dict(
            torch.load(fst_dir / "fst_projection.pth")
        )
...
def sampling(args, pipe, content_image=None, style_image=None, style_source_image=None):
    ...
    if not args.demo:
        Path(args.save_image_dir).mkdir(parents=True, exist_ok=True)
        # Saving sampling config
        save_args_to_yaml(
            args=args, output_file=Path(args.save_image_dir) / "sampling_config.yaml"
        )
    if args.seed:
    ...
    with torch.inference_mode():
        ...
        if args.save_image:
            ...
            save_single_image(save_dir=Path(args.save_image_dir), image=images[0])
            if args.character_input:
                save_image_with_content_style(
                    save_dir=Path(args.save_image_dir),
                    image=images[0],
