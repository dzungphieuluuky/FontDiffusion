...
class SimplifiedShapeTest:
    ...
    def print_results(self):
        ...
        # Summary
        mismatches = [
            (k, v) for k, v in self.results.items() if v.get("match") is False
        ]
...
def test_complete_pipeline():
    ...
    tester.log(
        "STYLE_ENC", "orig_style_feat", orig_style_feat, (batch_size, 1024, 3, 3)
    )
    tester.log("STYLE_ENC", "orig_style_vec", orig_style_vec, (batch_size, 1024))
    tester.log(
        "STYLE_ENC", "orig_style_hidden", orig_style_hidden, (batch_size, 9, 1024)
    )
    ...
    transformation_features = torch.randn(
        batch_size, num_queries + last_spatial_size, 1024
    )
    ...
    tester.log(
        "PROJECTION", "fst_condition", fst_condition, (batch_size, 292, cross_attn_dim)
    )
    tester.log(
        "PROJECTION",
        "orig_style_projected",
        orig_style_projected,
...
def test_fst_detailed():
    ...
    print(f"\nConcatenation:")
    print(
        f"  Input: {num_scales} tensors of ({batch_size}, {num_queries}, {query_dim})"
    )
